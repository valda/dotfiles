...
Treat it as the fast middle layer between `rg` and Serena/LSP:

- Use the `Grep` tool (or `rg` via Bash) first for broad text, filename, and
...
- Use `ast-grep` when the next question is syntactic shape, such as "calls to
  ...
  this kind".
- Use Serena MCP (`mcp__plugin_serena_serena__*`) when symbol identity,
...
1. Start with intent and language.
   ...
   known, use the `Grep` tool first to discover likely files or names.
...
3. Escalate to LSP or a symbol-aware tool when identity matters.
   ...
   a change is type-safe, use Serena MCP (`mcp__plugin_serena_serena__*`) after
   narrowing candidates with `ast-grep`.
