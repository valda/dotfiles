...
**対応コメント（`@codex 指摘反映しました…` のような ack コメント）は投稿しない**。
push 自体が Codex bot の再レビュートリガになるため通知として冗長で、さらに bot がその
返信を書くためにトークンを浪費する。修正意図はコミットメッセージに書けば必要な情報は
伝わる。

**不採用時**

- PR コメントに反証・理由を投稿（必要なら bot の該当コメントに 👎 reaction）。
  反証は「push しない」ケースなので Codex に意思表示する唯一の経路であり、これは残す。
