...
**指摘を「観測」と「修正案」に分解し、採るのは観測だけ**

codex は読みが浅く、対象の運用実態も設計意図も知らない。**こちらの漏れ・見逃しを拾うセンサーとしては有用だが、修正案の質はそこに追いつかない**ので、両者を切り離して扱う。

- **観測**（「ここは X という前提が要る」「Y が Z と食い違う」）→ 裏を取って評価する。bot の価値はここだけ
- **修正案**（「両方に入れろ」「この順序に変えろ」）→ **既定で捨てる**。観測が正しくても案が正しいとは限らない。解は自分で設計する

観測は当たりだが案は外れ、が最頻出パターン。bot は現状の構成を所与として穴を塞ぐ方向にしか動かないため、**前提そのものを消す解**（構成要素を系から外す / 機能を無効化する / そもそも不要と判断する）を構造的に見落とす。案をそのまま採ると触る面が増え、こちらの方針と衝突することすらある。

**より怠惰に、より傲慢に扱う。** 怠惰 = 直す量が最小になる解を自分で探す（bot の案は大抵それより大きい）。傲慢 = 設計判断は自分のほうが正しいと構える（bot は差分の周辺しか読んでいない）。**採用したのが観測だけで、直し方が bot の案と違うのは正常**。
